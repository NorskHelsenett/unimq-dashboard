...
## RE-REVIEW STATUS (baseline `6fc2fe6` → `9ee5e4a`, eleventh pass)

`go build ./...`, `go vet ./...` and `go test ./...` all pass.

**G22 and G23 are both fixed.** `timehelper` now parses RFC3339 — the same format `time.Time` marshals to — so a `MaintenanceEntry` round-trips cleanly, and an omitted `updated_at` defaults to `time.Now()` instead of being rejected. `UpdatedBy` is now populated from the request context.

**Two follow-ups from the same change**, both Low and both documentation-side: the `PatchMaintenanceEntry` Swagger example still shows the *old* space-separated format (**G36**), and `internal/docs/` is stale again (**G33**, second recurrence in three passes).

⚠️ *Prior pass:* G34 and G35 were Critical regressions that sat on the branch for two commits while `go build`, `go vet` and the whole suite stayed green. A renamed route parameter and a missing `!` each took out a subsystem. Still untested — see **O1**.
...
|---|------------------|-------|--------|-----|
| **G36** | **The `PatchMaintenanceEntry` Swagger example can't be submitted.** The `example` tags still carry the pre-RFC3339 format `2024-06-01 10:00:00`, but `Validate()` now parses with `time.RFC3339`. The sibling structs (`MaintenanceEntry`, `PostMaintenanceEntry`) were updated; this one was missed. | `internal/models/maintenance.go:192-193` | **Low.** Verified: feeding the example verbatim into `Validate()` returns `invalid start time format: parsing time "2024-06-01 10:00:00" as "2006-01-02T15:04:05Z07:00"`. Anyone who clicks "Try it out" on PATCH and submits the prefilled body gets a 400. | Change both `example` tags to `2024-06-01T10:00:00Z` / `2024-06-01T12:00:00Z` and regenerate the docs. |
| **G33** | **No guard against the committed Swagger spec drifting — and it has drifted again.** The RFC3339 example change was not followed by a `swag init`. | `internal/docs/{docs.go,swagger.json,swagger.yaml}`; `.github/workflows/testandbuild_backend.yaml` | **Low.** Verified: a fresh `swag init` differs from the committed spec — `models.MaintenanceEntry` and `models.PostMaintenanceEntry` still advertise `start`/`end` as `2024-06-01 10:00:00` while the source now says `2024-06-01T10:00:00Z`. Clients generated from the committed spec get the wrong format. This is the **second recurrence in three passes**, which is the argument for automating it. | Add a CI step that runs `swag init` and fails if `git diff --exit-code internal/docs` is non-empty. |
