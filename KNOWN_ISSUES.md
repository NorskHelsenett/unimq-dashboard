...
## RE-REVIEW STATUS (baseline `453440c` → `0e88062`, second pass)

`go build ./...` and `go vet ./...` pass. **`go test ./...` still FAILS** — `internal/database.TestDatabaseConnection` is red (see G3).

Twelve issues are cleanly fixed. **Five regressions remain open (G1–G5), two of them Critical crash paths.** The second round of fixes closed G6 and G7 correctly but did not touch G1–G5.

### 🚨 Regressions introduced by the fixes — fix these first

| # | What happens now | Where | Impact | Fix |
|---|------------------|-------|--------|-----|
| **G1** | **Webhook notifications are never sent, and the checker panics.** `SendWebhooks` was rewritten to build a request with `http.NewRequestWithContext` but **no `client.Do(req)` call was ever added** — the request is constructed and discarded. It then reads `req.Response.StatusCode`; `req.Response` is **nil** on a client request, so this nil-derefs. | `internal/helpers/notificationhelper/notificationhelper.go:22-34` | **Critical.** All webhook alerting is dead. The panic fires in the checker goroutine, which has **no `Recoverer`** → the whole process crashes on the first alarm or maintenance notice. Reproduced against a live, healthy webhook endpoint. | Add `resp, err := client.Do(req)`, `defer resp.Body.Close()`, and read `resp.StatusCode`. Drop `req.Body.Close()` and the `defer cancel()` inside the loop. |
| **G2** | **Any failed outbound RabbitMQ call panics.** The `defer resp.Body.Close()` added to fix R1 is registered **before** the `if err != nil` check. When `Do` fails, `resp` is nil. | `internal/clients/rest/rest.go:153-159` | **Critical.** RabbitMQ being briefly unreachable turns a clean error into a nil-deref. In a handler it's masked by `Recoverer` as a 500; in the **checker goroutine it kills the process**. Reproduced. | Move the `defer` to after the `err != nil` return. |
| **G3** | **Mongo credentials with `@` produce an unparseable URI**, and the existing unit test now fails. `url.QueryEscape` → `url.PathEscape` was the wrong swap: `PathEscape` does not escape `@`. `p@ssw0rd` renders as `mongodb://user@name:p@ssw0rd@localhost:27017`. | `internal/database/database.go:150-157` | **High.** Any password containing `@` breaks the connection. CI is red. The `strings.TrimPrefix` half of this fix is correct and does resolve B7. | Build the URI with `url.UserPassword(user, pass).String()`, or revert to `QueryEscape` for the userinfo only. Also trim `mongodb+srv://`. |
| **G4** | **Alarm rules created via the API can no longer be created at all.** The `return nil, fmt.Errorf("generic failure...")` added to fix B10 sits inside `if err != nil` but **outside** the `errors.Is(err, mongo.ErrNoDocuments)` branch, so the path that *successfully creates* a new notification host falls straight through into the error return. | `internal/api/v1/internal.go:51` | **High.** First-ever rule/recipient creation for a vhost always returns 500, even though the document was written. | Put the generic return in an `else` branch, or `return vhostNotification, nil` at the end of the `ErrNoDocuments` block. |
| **G5** | **Alarm notifications leak across vhosts.** `urls`/`emails` are now accumulated *outside* the vhost loop but `checkRule(..., urls, ...)` is still called *inside* it, so vhost #3's alarms are sent to the webhooks of vhosts #1 and #2 as well. | `internal/notify/checker.go:144-174` | **High** (confidentiality). Alarm bodies contain vhost and queue names. Grows with vhost count. | Pass `vhost.WebhookURLs()` to `checkRule`; keep the accumulated slices solely for `checkMaintenanceSchedules`. |

**G6 ✅ and G7 ✅ are resolved** — see below.

### ✅ Fixed and verified

B4 (missing `return`) · B6 (`Limits` now assigned) · B9 (pointer fields on `AlarmRuleUpdate`) · B12 (`middleware.GetReqID`) · B13 (`RequestID` before `Logger`) · S1 (checked `groups` assertion, uses `ErrInvalidGroupsType`) · R8 (`db.Close`) · R9 (`wg.Go` only) · R10 (`err` scoped to the goroutine) · A11 (`/api/healthz` + `/api/readyz`, unauthenticated).

Resolved in the second round:

- **G6 ✅** — `InsertAlarmEntries` now passes `options.UpdateOne().SetUpsert(true)`. Since `AlarmEntry` is only `_id` + `entries`, the upserted document is complete, so alarm history is genuinely persisted now. This also finally closes **B1**. (`AddAlarm` is now only reachable from `seeding.go` — dead in production, see X-list.)
- **G7 ✅** — `WithEmailConfig` option added and wired in `main.go`; `SendEmail` nil-guards `config` and returns the new sentinel `ErrEmailNotConfigured`, which `checkMaintenanceSchedules` handles with a `Warn` + `continue` rather than an error log. Clean.

### ⚠️ Partially fixed

- **B2** — maintenance notification is now sent once per tick instead of per vhost. Correct, but it created **G5**.
- **B3** — email works on the *maintenance* path now (**G7** resolved). `NotifyAlarm` is still webhook-only, so **alarm** emails remain unimplemented.
- **B7** — the `strings.TrimPrefix` half is correct and resolves the malformed-URI bug; the escaping half introduced **G3**.
- **B8** — `evaluate` now returns `*float64`, but `v := new(float64)` means the trailing `if v == nil` is **dead code**. A missing queue still yields `0`, so a rule with `threshold <= 0` still false-fires. Note that if it *did* return nil, `checkRule` would nil-deref at `*evalResult.Value` (`checker.go:262`) — that dereference needs a guard first.
- **R11** — `WriteTimeout` is now 90s, comfortably above `middleware.Timeout(60s)`, so the handler timeout can produce its 504 before the write deadline fires. The race is gone. Still missing `ReadHeaderTimeout` and `IdleTimeout` (Slowloris exposure).

### ❌ Explicitly attempted but still broken

**B8** (above), **B10** (see **G4**), **R1** (see **G2**), **R2** (see **G1**).

### Not yet addressed

B5, B11, B14–B19, B21–B23 · S2–S12 · R3–R7 · D1–D5 · X1–X8 · A1–A10, A12 · N1–N9 · O1–O5 — plus all FRONTEND, HELM, DOCKER, CI/CD and DEV-ENV items. Note **H4** (TCP-only probes) can now be closed out in the chart, since A11 gave it real endpoints to probe.
...
| B10 ⚠️ Regressed | Non-`ErrNoDocuments` DB errors are swallowed | `internal/api/v1/internal.go:34-50` | In `ensureNotificationHostExists`, the `if errors.Is(err, mongo.ErrNoDocuments)` block has no `else` and the outer `if err != nil` never returns. A real DB failure falls through to a second query. | **Medium** | Return the error when it isn't `ErrNoDocuments`. |
| B11 | `LOG_LEVEL` can never be set *less* verbose than Info | `internal/logger/logger.go:34-36` | `UpdateLogLevel` returns early if `slog.Default().Enabled(ctx, level)` is true. Default is Info, so Warn/Error are already "enabled" and the update is skipped. | **Medium** | Always rebuild the handler; drop the guard. |
...
| B18 | Writes `rules.$.lastResolved`, a field the model doesn't have | `internal/database/notificationrules.go:124` | Written on every resolve but never read or returned — schema drift. | **Low** | Add `LastResolved *time.Time` to `AlarmRule` or drop the write. |
...
| S8 | No request body size limit | `internal/routes/httpsuite/response.go:86-99` | `ReadResponse` decodes straight from `r.Body`. An arbitrarily large body can exhaust memory. | **Medium** | Wrap with `http.MaxBytesReader`. |
...
| S11 | RabbitMQ basic-auth credentials may go over plaintext HTTP | `internal/config/config.go:66` default `http://localhost`; no TLS enforcement in `rest.go` | Credentials are sent in a `Basic` header; with an `http://` host they're in cleartext. | **Medium** | Require `https://` outside local dev; make TLS config explicit. |
...
| # | Issue | Where | Why it matters | Severity | Potential fix |
|---|-------|-------|----------------|----------|---------------|
| R1 ⚠️ Regressed | Response body leaked on every non-2xx upstream response | `internal/clients/rest/rest.go:167-175` | The `StatusCode > 399` branch reads the body and returns without `Close()`. Same on the `io.ReadAll` error path and the JSON-decode error path in `handleOutput`. Leaks a connection per RabbitMQ error. | **High** | `defer resp.Body.Close()` immediately after `Do`. |
| R2 ⚠️ Regressed | Webhook POSTs have no timeout and no context | `internal/helpers/notificationhelper/notificationhelper.go:18` | Bare `http.Post` uses the default client (no timeout). One hung webhook blocks the checker goroutine indefinitely, stalling **all** alarm evaluation. | **High** | Use a client with a timeout and `http.NewRequestWithContext`. |
...
| R6 | `writeJSONResponse` can recurse infinitely on write failure | `internal/routes/httpsuite/response.go:73-84` | The error path calls itself, which writes again, fails again, recurses again. Also calls `WriteHeader` a second time. | **Medium** | Log and return; never re-enter. |
