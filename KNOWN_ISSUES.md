...
## RE-REVIEW STATUS (baseline `41974b8` → `7b60670`, thirteenth pass)

`go build ./...`, `go vet ./...` and `go test ./...` all pass, and `task go:swag-verify` reports the spec in sync.

**Four things fixed:** the **G33** drift check landed and works; **A1** json tags; **B14**'s dead `MaintenanceAdminResponse`; and `SendWebhook` now derives its timeout from the caller's context instead of `context.Background()`, so the checker's shutdown actually propagates.

🚨 **Three new regressions, none caught by build, vet, tests or `tsc`:**
- **G37** — the `{rule-id}`/`{recipient-id}` rename broke five endpoints. This is **G34 happening a second time, in the same way, three passes later.**
- **G38** — removing `url.QueryUnescape` broke the RabbitMQ default vhost `/`.
- **G39** — the json tag rename silently empties four frontend views.

⚠️ **G38 is my fault.** The old **A4** entry asserted that `chi.URLParam` always returns a decoded value and the unescape was redundant. That was reasoned from reading, never measured, and it is wrong — chi routes on `RawPath`, so `%2F` comes back encoded. A4 has been rewritten with the measured behaviour. Acting on it caused the regression.

**The pattern worth acting on:** G34 and G37 are the same bug, and the audit that catches both is one line — compare the params declared in `internal/api/routes/` against the names read by `chi.URLParam`. As a test that is a few dozen lines and ends this class of regression permanently. See **O1**.
...
|---|------------------|-------|--------|-----|
| **G37** | **Five notification endpoints return 400 — an exact repeat of G34.** `b623b38` renamed the route params to `{rule-id}` and `{recipient-id}`, but four rule handlers still read `chi.URLParam(r, "rule")` and one recipient handler still reads `"recipient"`. chi returns `""` for an undeclared name, so each trips its own "missing required id" guard. | routes: `internal/api/routes/v1/apiservice.go:71-79`; readers: `notificationrule.go:49,202,254,331` and `notificationrecipient.go:215` | **Critical.** Verified against the real route tree with admin claims: `GET`/`POST`/`DELETE /notifications/{v}/rules/{id}`, `POST .../rules/{id}/toggle` and `DELETE .../recipients/{id}` all return `400`. Only `/rules/{id}/test` works, because it alone reads `"rule-id"`. A whole-tree audit shows handlers read `rule` and `recipient`, which **no route declares**. | Update the five call sites to `"rule-id"` / `"recipient-id"`. The one-line audit that found this: compare `grep -rho '{[a-z-]*}' internal/api/routes/` against `grep -rho 'URLParam(r, "[a-z-]*"' internal/api/`. |
| **G38** | **The RabbitMQ default vhost `/` is broken on the notification-rule endpoints.** `e6a6d15` removed `url.QueryUnescape` from `notificationrule.go` on the premise that chi already decodes. It does not. | `internal/api/v1/notificationrule.go` (unescape removed); every other handler still has it | **High.** Verified against a real chi router: `/vhosts/%2F` yields the param `"%2F"`, *not* `"/"` — chi routes on `RawPath` when set. The frontend sends `encodeURIComponent(vhost)`, so the default vhost `/` arrives as `%2F` and the handler now queries Mongo for a vhost literally named `"%2F"`, which never matches. `%20` happens to work (chi decodes that one), which makes it look fine in casual testing. | Restore unescaping here, and use `url.PathUnescape` everywhere so a vhost named `a+b` is not mangled into `a b`. See the corrected **A4**. |
| **G39** | **The JSON tag rename silently empties four frontend views.** `0380572` changed the wire format from Go-default PascalCase to lowercase (`Name`→`name`, `Recipients`→`recipients`, `Rules`→`rules`, `Scheduled`→`scheduled`, `History`→`history`), but no frontend code was updated. | backend: `models/notification.go:280-283`, `models/maintenance.go:246-248`; frontend: `DashboardActiveRecipientsWidget.tsx:18,47`, `DashboardAlarmsSummaryWidget.tsx:122,147,151`, `pages/Notifications.tsx:13-14`, `services/maintenance.ts:17,31` | **High.** Nine call sites still read the old keys. Each is guarded by `?? []` or `?? ''`, so nothing throws — the dashboard recipients widget, the alarms summary, the notifications page and the maintenance scheduled/history lists just render **empty**. The TS interfaces in `types/notifications.ts:5` and `types/maintenance.ts:5` still declare `Recipients`/`Scheduled`, so `tsc` cannot catch it either. | Rename the fields in the TS interfaces and the nine call sites. The backend change itself is correct and worth keeping (it closes A1). |
...
| A4 ⚠️ Corrected | Inconsistent URL-unescaping of path parameters | `api/v1/notificationrule.go` (none), vs `notification.go`, `notificationrecipient.go`, `internal.go`, `alarm.go`, `rmq/vhosts.go`, `rmq/queues.go` (one each) | **The original entry here was wrong and this correction supersedes it.** It claimed `chi.URLParam` always returns a decoded value, so the unescape was redundant. Measured against a real chi router, that is false: chi routes on `r.URL.RawPath` when it is set, so `/vhosts/%2F` yields the param `"%2F"` — **still encoded**. The unescape is required for the RabbitMQ default vhost `/`. It is genuinely wrong only in that `QueryUnescape` also turns `+` into a space, mangling a vhost literally named `a+b`. | **Medium** | Use `url.PathUnescape`, not `QueryUnescape`, and apply it uniformly at every path-parameter read. See **G38** for the endpoints that now have none. |
...
Swagger now drives an OAuth2 **authorization code + PKCE** flow against Dex (`unimq-swagger` public client, `trustedPeers` on `unimq-dashboard` so one token is accepted by both). The resource-owner password flow and `LoginHandler` were removed, and the backend flow's `state` parameter is now generated, cookie-bound and validated (U1, fixed). The items below are what remains.
...
### Added in the eleventh pass

| # | What was wrong | How it was fixed | Where it lives now |
|---|----------------|------------------|--------------------|
| **G22** | A `MaintenanceEntry` could not parse its own JSON output. Marshalling used `time.Time`'s default RFC3339, but `UnmarshalJSON` parsed via `timehelper.ParseTimeInUTC`, whose layout was `2006-01-02 15:04:05` — so any round-trip (client replay, fixture, cached copy) was rejected with `invalid start time format`. | `ParseTimeInUTC` was replaced by `ParseTimeInRFC3339` (`time.Parse(time.RFC3339, …)`) and every call site updated, including the `PatchMaintenanceHandler`. The `example` tags on `MaintenanceEntry` and `PostMaintenanceEntry` were moved to RFC3339 to match. Verified: marshal → unmarshal now succeeds and preserves `start`, `end` and `updated_at` exactly. | `internal/helpers/timehelper/timehelper.go:5-7`; call sites at `internal/models/maintenance.go:74,79,146,153,158,206,211` and `internal/api/v1/maintenance.go:281-282` (commits `cd7865c`, `9ee5e4a`) |
| **G23** | `UnmarshalJSON` hard-rejected an empty `updated_at`, even though it is an audit field the client should never have to supply — and the struct tag was `omitempty` on a `time.Time`, which `encoding/json` ignores, so the field could not be omitted either way. | The required-check was replaced by a default: `updated_at` is parsed only when present (`if aux.UpdatedAt != ""`), otherwise it falls back to `time.Now()`. The misleading `omitempty` is gone, and `UpdatedBy` is now filled from the request context rather than trusted from the body. | `UnmarshalJSON` — `internal/models/maintenance.go:144-150,172`; struct tag at `:114`; `ToMaintenanceEntry(ctx)` — `:88-103` (commits `ed196fd`, `d77138a`, `9ee5e4a`) |

### Added in the twelfth pass

| # | What was wrong | How it was fixed | Where it lives now |
|---|----------------|------------------|--------------------|
| **U1** | The backend authorization-code flow passed the literal string `"state"` to `AuthCodeURL` and the callback never read `state` back. That is the flow's entire CSRF defence, so a fixed, unchecked value provided none — an attacker could feed a victim's browser a callback URL bearing their own authorization code and graft the attacker's identity onto the victim's session. | `RedirectHandler` now mints 256 bits from `crypto/rand`, stores it in a single-use `HttpOnly`, `SameSite=Lax`, 10-minute cookie (`Secure` derived from `r.TLS` or `X-Forwarded-Proto`, since TLS terminates at the ingress), and passes it as `state`. `OauthCallbackHandler` clears the cookie, then requires both the query parameter and the cookie and compares them with `subtle.ConstantTimeCompare` **before** exchanging the code. `SameSite=Lax` was chosen deliberately: it still rides the top-level GET redirect back from Dex, but not cross-site subresource requests. | `newOAuthState`, `setStateCookie`, `clearStateCookie`, `verifyState` — `internal/clients/dex/dex.go:175-232`; handlers at `:249-277`; tests in `internal/clients/dex/dex_test.go` (commits `c1c9ecc`, `41974b8`) |
| **G36** | The `PatchMaintenanceEntry` Swagger example still carried the pre-RFC3339 format `2024-06-01 10:00:00` after `Validate()` moved to `time.RFC3339`, so submitting the prefilled "Try it out" body returned a 400. Its two sibling structs had been updated; this one was missed. | Both `example` tags were changed to RFC3339 and `internal/docs/` regenerated. Verified: `swag init` now produces zero diff across `swagger.json`, `swagger.yaml` and `docs.go`. | `internal/models/maintenance.go:191-192` (commits `8b63f55`, `41974b8`) |

**Note on the U1 patch:** the first version used `defer clearStateCookie(w, r)`, which reads correctly but is wrong — the deferred call fires *after* `http.Redirect` or the error response has already flushed headers, so the `Set-Cookie` was silently discarded and the state stayed replayable. `TestCallbackClearsStateCookie` caught it on the first run. The clear is now an eager call at the top of the handler.

### Added in the thirteenth pass

| # | What was wrong | How it was fixed | Where it lives now |
|---|----------------|------------------|--------------------|
| **G33** | Generated Swagger artefacts are committed, but nothing compared them against a fresh generation. CI ran `make docs-run` and discarded the result, so the spec drifted silently — twice in four passes. | A `go:swag-verify` task regenerates the docs and fails if `internal/docs` comes back dirty; the workflow calls it instead of `make docs-run` (the Makefile is no longer maintained). Three supporting changes made the check trustworthy: `swag` is pinned to the `go.mod` version (`v1.16.4` local / `v1.16.6` go.mod / `@latest` CI were all in play, so an unpinned check fails on version skew, not drift); the `swag` task moved off `deps:`, which Task runs in **parallel**, so `swag-format` and `swag-generate` were racing; and `swag fmt` is excluded from verify because it rewrites annotation alignment in source. Verified both directions in a throwaway clone — a stale-docs commit fails with a non-zero exit, a regenerated one passes. | `swag-verify`, `swag-check`, `swag` — `taskfiles/go.yaml:94-122`; `.github/workflows/testandbuild_backend.yaml:35-44` (commit `ebc6987`) |
| **A1** | Response models carried no `json` tags, so the API emitted Go-default PascalCase (`Name`, `Recipients`, `Scheduled`, `AlarmID`, `Database`/`Dex`/`RabbitMQ`) while the rest of the API was lowercase — two conventions on one surface. | `json` tags were added to `VhostNotification`, `AlarmEntry`, `MaintenanceResponse` and `HealthStatus`. *(The backend half is correct; the frontend was not updated to match — see **G39**.)* | `models/notification.go:280-283`, `models/alarms.go:10-11`, `models/maintenance.go:246-248`, `models/health.go:10-14` (commit `0380572`) |
| **B14** | `MaintenanceAdminResponse` and its constructor were dead code — defined, exported and never referenced. | Both were deleted. | Removed from `internal/models/maintenance.go` (commit `0380572`) |
| — | `SendWebhook` built its 10-second timeout from `context.Background()`, discarding the caller's context. Cancelling the checker could not interrupt an in-flight webhook, so shutdown blocked until every request timed out on its own. | The timeout now derives from the passed `ctx`. | `SendWebhook` — `internal/helpers/notificationhelper/webhook.go:32` (commit `7b60670`) |
