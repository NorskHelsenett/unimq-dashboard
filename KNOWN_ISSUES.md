...
## RE-REVIEW STATUS (baseline `fc6b9ea` → `7efe67b`, eighth pass)

`go build ./...`, `go vet ./...` and **`go test ./...` all pass** (run as three separate commands).

**G18 and G19 are both fixed.** `checkMaintenanceSchedules` now captures `SendWebhooks` into `webhookStatus` and reports per-URL, and `ReadyzHandler` has the missing `return`.

**The G18 fix moved `SetMaintenanceEntryNotified` to the end of the loop body, which introduced a regression — G31 below.** The reorder also exposed that the "email disabled" configuration doesn't do what `.env.example` promises (**G32**).

*Earlier context:* the seventh pass covered a large restructuring — Prometheus removed end-to-end, RabbitMQ handlers split into `internal/api/v1/rmq/`, routes moved to `internal/api/routes/`, `httpsuite` moved to `internal/api/httpsuite/`, notifications split into `email.go`/`webhook.go` with per-recipient status reporting, and Swagger given an OAuth2 authorization-code + PKCE flow. It closed B8, B20, S7, S8, X1, X3, X4, A3, R2, G14, G15, G16 and the `NotifyAlarm` early-return.

### 🚨 Still open — fix these first

| # | What happens now | Where | Impact | Fix |
|---|------------------|-------|--------|-----|
| **G31** | **Regression from the G18 fix: maintenance notifications can loop forever.** `SetMaintenanceEntryNotified` was moved to *after* the email block, but the `ErrEmailNotConfigured` branch still does a bare `return` — which now skips the marking entirely. | `internal/notify/checker.go:327-330` vs `:338-340` | **High.** The entry is never marked notified, so the next tick reprocesses it and **re-sends every webhook**, every interval, forever. Before the reorder the entry was marked first, so this could not happen. The `return` also aborts the whole loop, so no later entry is ever processed. | `continue` instead of `return`, and mark as notified once delivery has been attempted — or only mark when at least one channel succeeded (see **G30**). |
| **G32** | **"Email disabled" is never actually detected.** `.env.example:37` promises *"If EMAIL_SMTP_HOST is empty, email sending will be disabled"*, but `sendEmail` only returns `ErrEmailNotConfigured` when `config == nil` or `EmailFromAddress == ""` — it never checks `EmailSMTPHost`. `EmailFromAddress` defaults to `unimq@example.com`, so it is effectively never empty. | `internal/helpers/notificationhelper/email.go:31-40`; `config.go:70,74` | **Medium.** Verified: with `EMAIL_SMTP_HOST=""` and the default FROM, every notification attempts a real SMTP dial and fails with `dial tcp :25: connect: connection refused` — one dial per recipient per tick — instead of being skipped. `EmailConfig.IsValid()` returns `false` for exactly this case but is never consulted at send time. | Guard on `config.IsValid()` (or `EmailSMTPHost == ""`) in `sendEmail`, and return `ErrEmailNotConfigured`. |
| **G20** | **`GET /api/v1/rabbitmq/usage` is permanently broken.** It is routed to `GetRMQVhostUsageHandler`, which requires `chi.URLParam(r, "vhost")`, but that route has no `{vhost}` segment. | `internal/api/routes/v1/rmq.go:26` | **Medium.** Verified by exercising the real route tree: returns `400 {"message":"vhost name is required"}` on every call. The endpoint is also absent from the Swagger spec. | Either add a cluster-wide usage handler or drop the route. |
| **G21** | **Swagger documents a vhost-usage path that does not exist.** The annotation says `/v1/vhost/{vhost-name}/usage` (singular `vhost`); the real route is `/v1/vhosts/{vhost}/usage`. | `internal/api/v1/rmq/vhosts.go:163`; `internal/docs/swagger.json:1761` | **Medium.** Verified: the documented path returns **404**. "Try it out" in Swagger UI fails for this endpoint. | Fix the `@Router` annotation to `/v1/vhosts/{vhost-name}/usage` and regenerate. |
...
- **G30 — maintenance is marked notified regardless of delivery.** *Improved but not closed.* `SetMaintenanceEntryNotified` now runs **after** sending rather than before, but it is still called unconditionally, so an entry whose webhooks and emails all failed is still recorded as notified and never retried. (The `ErrEmailNotConfigured` path has the opposite problem — see **G31**.) `internal/notify/checker.go:338-340`.
...
| A10 ⚠️ Partial | Swagger annotations don't match the routes | **Fixed** for the `apiservice` routes (`{vhost-name}`, `{rule-id}`, `{maintenance-id}` now match). **Still wrong** for: the RMQ routes (`{vhost}`/`{queue}` registered vs `{vhost-name}`/`{queue-id}` documented); `status.go:19` documents `/v1/checker/status` but the route is `/api/v1/status`; `health.go` documents `/healthz` and `/readyz` but they are served at `/api/healthz` and `/api/readyz`; and see **G21**. | Generated docs and clients are wrong; "Try it out" 404s. | **Medium** | Align annotations with `internal/api/routes/` and regenerate. |
