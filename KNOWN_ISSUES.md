...
## RE-REVIEW STATUS (baseline `453440c` → `a0fc2bd`, fifth pass)

`go build ./...` passes and `go test ./...` passes, but **`go vet ./...` now FAILS** (see G10).

**All four regressions from the previous pass (G1a, G1b, G8, G9) are fixed.** The database layer was substantially reworked to check query results, which is the right direction — but the specific counts chosen are wrong in two ways, verified against a real MongoDB 7 instance.

Every fixed issue, with its cause and remedy, is recorded in the tables at the **bottom of this document**. Backlog tables above contain only what is still outstanding.

### 🚨 Still open — fix these first

| # | What happens now | Where | Impact | Fix |
|---|------------------|-------|--------|-----|
| **G11** | **Every alarm event after the first reports a failure.** `InsertAlarmEntries` treats `UpsertedCount == 0` as "not found", but `UpsertedCount` is only non-zero when the upsert *creates* the document. Appending to an existing alarm returns `Matched=1, Modified=1, Upserted=0`. | `internal/database/alarms.go:115` | **High.** The `$push` actually succeeds, so data is fine, but the caller is told it failed — the checker logs a spurious `ErrAlarmNotFound` on every subsequent alarm transition. Verified against MongoDB 7. | Check `results.MatchedCount == 0 && results.UpsertedCount == 0`. |
| **G12** | **Idempotent updates are reported as "not found".** Thirteen call sites treat `ModifiedCount == 0` as failure. MongoDB returns `Matched=1, Modified=0` when a `$set` writes the value a field already holds. | `internal/database/notificationrules.go:74,114,156,197,234,272`; `notificationrecipients.go:64,105`; `maintenance.go:155,192,277,319` | **High.** Re-enabling an already-enabled rule, re-saving an unchanged threshold, or re-marking a maintenance entry `notified` all return an error and surface as a failed API call. The checker hits this on every repeat tick. Verified against MongoDB 7. | Use `MatchedCount == 0` for "not found" — as `notifications.go:85` already does correctly. Keep `ModifiedCount` for logging only. |
| **G10** | **`go vet` fails — and the user sees a formatting artefact.** `fmt.Sprintf` is called with the `%w` verb, which it does not support. | `internal/api/v1/maintenance.go:152` | **Medium**, but it breaks any CI step running `go vet`. The client receives the literal `bad time format, %!w(*errors.errorString=&{...})`, which also re-leaks internal error text that **S2** just finished removing. | Use `%v`, or drop the interpolation and rely on `WithError(err)`. |
| **G13** | **The new `Recipient` validation tags are inert.** They are spelled `validation:"..."`; go-playground/validator reads `validate:"..."`. | `internal/models/notification.go:46-50` | **Low** in practice — handlers validate `PostRecipient` (which is tagged correctly), so nothing regressed; but the tags give a false impression of coverage. Verified: the `validation:` spelling returns no error for an invalid email. | Rename the tag key to `validate`. |

### ⚠️ Worth a second look

- **A2 (validation coverage)** — `IsRequestValid` is still called from only one handler (`AddNotificationsRecipientHandler`). `PostAlarmRule` has no `validate` tags, so negative thresholds and empty queue names are still accepted, and no maintenance model is validated.
- **A3** — unchanged. `NewValidationErrors` still ignores the `errors.As` result, so a non-validator error yields an empty error map.
- **D2** — partially addressed. Sentinels (`ErrAlarmNotFound`, `ErrMaintenanceNotFound`, `ErrRecipientNotFound`, `ErrNotificationRuleNotFound`, `ErrVhostNotFound`) now exist and 6 handler sites map them to 404, but 14 `StatusNotFound` sites in total means the mapping is not yet uniform.
- **D1** — the intent (check query results) is now implemented everywhere, but the counts are wrong; see **G11**/**G12**. Re-open once those are corrected.
- **R7** — unchanged. `writeJSONResponse` still writes a nil body with a success status when marshalling fails.
- **S11** — unchanged. Defaults are `https://localhost` while `.env.example` still says `http://localhost`, and both services speak plain HTTP on their default ports. With Prometheus connectivity a hard startup requirement, the built-in defaults still prevent boot.
- **Swagger typo** — `models/notification.go:42` now reads `@Type type of the recipient - "webhook", "webhook"`; the second should be `"email"`.
- **`internal/database/interface.go`** — the generic `Store[T]` now takes `context.Context`, which resolves the earlier concern. The remaining constraint is unchanged: **one concrete type cannot satisfy two instantiations** (`Store[AlarmEntry]` and `Store[Vhost]` both demand a method named `GetAll` with different signatures), so `*Database` cannot implement `AlarmHandler` and `VhostHandler` at once. Separate adapter types per entity are required. Also note `Store[T]` has no home for `Append`-style or multi-argument operations beyond the extra methods already being bolted on.
...
- `go build ./...` passes cleanly. (`go vet` regressed in the fifth pass — see **G10**.)

---

## FIXED — CHANGE LOG

Everything below has been verified fixed in the current tree (`a0fc2bd`), and has been removed from the backlog tables above. Locations are where the fix lives **today**. Rows marked *(partial)* are still open in the backlog.
